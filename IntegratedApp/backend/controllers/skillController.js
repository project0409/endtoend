...
exports.getSkillSetById = async (req, res) => {
  try {
    ...
    if (!skillSet) return res.status(404).json({ message: 'Skill set not found' });
...
exports.updateSkillSet = async (req, res) => {
  try {
    const skillSet = await SkillDatabase.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!skillSet) return res.status(404).json({ message: 'Skill set not found' });
    res.json({ success: true, skillSet });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
